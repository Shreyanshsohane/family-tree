let chek = "ankit";
const male_avatar =
  "https://images.pexels.com/photos/2876486/pexels-photo-2876486.png?auto=compress&cs=tinysrgb&dpr=1&w=500";
const female_avatar = "https://www.w3schools.com/howto/img_avatar2.png";

function nodeClick(data) {
  let dummyIds = window.localStorage.getItem("dummy_id");
  let finalDummyids = JSON.parse(dummyIds);
  let choosenDummyId = {};
  outerLoop: for (let i = 0; i < finalOutput?.length; i++) {
    for (let j = 0; j < finalDummyids.length; j++) {
      if (finalOutput[i].id == finalDummyids[j].id) {
        choosenDummyId = { id: finalDummyids[j].pid, dummy: true };
        break outerLoop;
      }
    }
  }
  if (!choosenDummyId.dummy) {
    if (data == 1 || data == "1") {
      let valueId = window.localStorage.getItem("changed_id");
      alert(valueId);
      choosenDummyId = { id: valueId, dummy: false };
      AvatarCreated.postMessage(JSON.stringify(choosenDummyId));
      // AvatarCreated.postMessage("ankit");
    } else {
      alert(data);
      choosenDummyId = { id: data, dummy: false };
      AvatarCreated.postMessage(JSON.stringify(choosenDummyId));
      // AvatarCreated.postMessage("ankit");
    }
  } else {
    alert(choosenDummyId);
    AvatarCreated.postMessage(JSON.stringify(choosenDummyId));
    // AvatarCreated.postMessage("ankit");
  }
}
...
function GenerateTree(e) {
  ...
  const memberElements = document.getElementsByClassName("member-view-box");

  for (let i = 0; i < memberElements.length; i++) {
    console.log("Member elements", memberElements);
    const id = memberElements[i].getAttribute("id");
    memberElements[i].addEventListener("click", () => nodeClick(id));
  }
}
function Member(e) {
...
function CreateNode(e) {
  ...
  var r = null == params?.template ? "circle" : params?.template,
    n = "";
  ("circle" != r && "rounded" != r) || (n = Template1(r, e)),
    "tilted" == r && (n = Template2()),
    "raised" == r && (n = Template3().replace("{shadow}", e.gender)),
    "" == e.photo &&
      (e.photo = "MALE" == e.gender ? male_avatar : female_avatar);
...
function Template1(e, pd) {
  var t = "";
  return (
    (t += "<a href=''>"),
    (t += `<div class='member-view-box' id='${pd.id}'>`),
    (t +=
